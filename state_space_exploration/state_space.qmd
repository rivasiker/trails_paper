...
Once the state space is generated, we can find all possible transitions between the different states by applying a set of rules. There can only be three different phenomena:
* A recombination event, where a two linked sites, one on the left and one on the right, become unlinked. 
* A reversible coalescent event, which is the opposite of a recombination event and, thus, left and right sites sitting in different chromosomes now become linked.
* An irreversible coalescent event, in which two sites at the same side (either left or right) coalesce. 
